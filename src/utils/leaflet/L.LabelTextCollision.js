L.LabelTextCollision = L.Canvas.extend({
	...
	_text: function (ctx, layer) {
		let text = layer.options.text
		if (text == null) return

		//文字样式
		let textStyle = layer.options.textStyle || {}
		...
		// label bounds offset
		let offsetX = textStyle.offsetX || 0
		let offsetY = textStyle.offsetY || 0
		...
		if (this.options.collisionFlg) {
			let textWidth = ctx.measureText(text).width + p.x // + offsetX;
			let textHeight = p.y + offsetY + 20
			let bounds = L.bounds(L.point(p.x + offsetX, p.y + offsetY), L.point(textWidth, textHeight))
			for (let index in this._textList) {
				let pointBounds = this._textList[index]
	...
	_getCenter: function (points) {
		...
		for (i = 0, dist = 0; i < len - 1; i++) {
			...
			if (dist > halfDist) {
				...
				let resutl = [p2.x - ratio * (p2.x - p1.x), p2.y - ratio * (p2.y - p1.y)]
