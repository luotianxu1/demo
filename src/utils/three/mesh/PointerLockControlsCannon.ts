...
class PointerLockControlsCannon extends THREE.EventDispatcher {
	...
	constructor(camera: THREE.PerspectiveCamera, cannonBody: CANNON.Body) {
		super()
	...
	getDirection() {
		...
		;(vector as any).applyQuaternion(this.quaternion)
		return vector
	}
	...
	update(delta: number) {
		...
		if (this.moveForward) {
			...
			// console.log(this.inputVelocity.z);
		}
		...
		this.yawObject.position.copy(this.cannonBody.position as any as THREE.Vector3)
	}
}
