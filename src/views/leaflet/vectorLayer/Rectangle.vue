<template>
	<div class="main">
		<div class="btnList">
			<el-button type="primary" @click="setBounds">设置矩形</el-button>
...
onMounted(() => {
	...
	let latlngs: L.LatLngBoundsExpression = [
		[54.559322, -5.767822],
		[56.1210604, -3.02124]
	]
	...
	rectangle.on("tooltipopen", e => {
		console.log(e, "tooltipopen")
	})
	// 当绑定到该层的 tooltip 关闭时触发。
	rectangle.on("tooltipclose", e => {
...
// 如果折线没有经纬度，则返回 true 。
const isEmpty = () => {
	console.log(rectangle?.isEmpty())
}
// 返回折线上最接近 p 的点。
const closestLayerPoint = () => {
	console.log(rectangle?.closestLayerPoint(L.point(45.51, -122.68)))
}
// 返回折线的中心坐标 (centroid) 。
const getCenter = () => {
...
const addLatLng = () => {
	rectangle?.addLatLng([45.51, -122.68])
}
...
const bindPopup = () => {
	rectangle?.bindPopup("<div>123<div/>")
}
...
const togglePopup = () => {
	rectangle?.togglePopup()
}
//如果该层绑定的弹出窗口当前已打开，则返回 true。
const isPopupOpen = () => {
	console.log(rectangle?.isPopupOpen())
}
// 设置绑定到该层的弹出窗口的内容。
const setPopupContent = () => {
	rectangle?.setPopupContent("<div>456789</div>")
}
// 返回绑定到该层的弹出窗口。
const getPopup = () => {
	console.log(rectangle?.getPopup())
}
// 将工具提示（tooltip）绑定到传入的图层 content 并设置必要的事件侦听器。如果一个 Function 被传递，它将接收图层作为第一个参数，并应返回 String 或 HTMLElement。
const bindTooltip = () => {
	rectangle?.bindTooltip("<div>789</div>")
}
...
const toggleTooltip = () => {
	rectangle?.toggleTooltip()
}
// 如果该层绑定的工具提示（tooltip）当前已打开，则返回 true。
const isTooltipOpen = () => {
	console.log(rectangle?.isTooltipOpen())
}
// 设置绑定到该层的工具提示（tooltip）的内容。
const setTooltipContent = () => {
	rectangle?.setTooltipContent("<div>12356<div/>")
}
// 返回绑定到该层的工具提示（tooltip）。
const getTooltip = () => {
	console.log(rectangle?.getTooltip())
}
</script>
