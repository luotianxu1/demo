...
onMounted(async () => {
	...
	let geoMapTileLayer = L.geoJSON(mapData, {
		...
		onEachFeature: function (feature, layer) {
			let html = "<div style='color:blue;cursor:pointer'>" + (layer as any).feature.properties.name
			layer.bindPopup(html)

			let centroid = turf.centroid(feature as any)
			let latlng: LatLngExpression = [centroid.geometry.coordinates[1], centroid.geometry.coordinates[0]]
			console.log("质点：", latlng)
			let popup = L.popup().setLatLng(latlng).setContent(html)
