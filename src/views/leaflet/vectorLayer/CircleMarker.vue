<template>
	<div class="main">
		<div class="btnList">
			...
			<el-button type="primary" @click="setLatLngs">设置位置</el-button>
...
onMounted(() => {
	...
	circleMarker = L.circleMarker([24.522422570142833, 118.11471659369913], {
		radius: 50, //圆形标记的半径，单位是像素
...
const toGeoJSON = () => {
	console.log(circleMarker?.toGeoJSON())
}
...
const setLatLngs = () => {
	circleMarker?.setLatLng([37, -109.05])
}
...
const bindPopup = () => {
	circleMarker?.bindPopup("<div>123<div/>")
}
...
const isPopupOpen = () => {
	console.log(circleMarker?.isPopupOpen())
}
// 设置绑定到该层的弹出窗口的内容。
const setPopupContent = () => {
	circleMarker?.setPopupContent("<div>456789</div>")
}
// 返回绑定到该层的弹出窗口。
const getPopup = () => {
	console.log(circleMarker?.getPopup())
}
// 将工具提示（tooltip）绑定到传入的图层 content 并设置必要的事件侦听器。如果一个 Function 被传递，它将接收图层作为第一个参数，并应返回 String 或 HTMLElement。
const bindTooltip = () => {
	circleMarker?.bindTooltip("<div>789</div>")
}
...
const isTooltipOpen = () => {
	console.log(circleMarker?.isTooltipOpen())
}
// 设置绑定到该层的工具提示（tooltip）的内容。
const setTooltipContent = () => {
	circleMarker?.setTooltipContent("<div>12356<div/>")
}
// 返回绑定到该层的工具提示（tooltip）。
const getTooltip = () => {
	console.log(circleMarker?.getTooltip())
}
</script>
